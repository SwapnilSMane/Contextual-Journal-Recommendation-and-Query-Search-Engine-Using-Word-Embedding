...
class Bert_embedding:
    ...
    def __init__(self, df_sentances):
        self.sentances = list(df_sentances['text']) # cleaned and corrected sentances
...
class glove50_embedding:
    ...
    def glove_vector(self):
        ...
        f = open('/home/swapnil/Documents/ML1/Project/Implementation/glove.6B/glove.6B.300d.txt', encoding='utf-8')
        for line in f:
            values = line.split()
            word = values[0]
    ...
    def query_embedding(self, query):
        query_vector= sum([self.word_embeddings.get(w, np.zeros((300,))) for w in query.split()])/(len(query.split())+0.001)
        self.query_vector = query_vector
        return query_vector
